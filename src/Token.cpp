...
Token Token::operator ==(Token& tk) {
    ...
    if(tk.tipo == ENTERO) return Token(tk.parse<int>() == this->parse<int>(), num_linea);
    if(tk.tipo == STRING) return Token(tk.parse<string>() == this->parse<string>(), num_linea);
...
Token Token::operator >(Token& tk){ return tk; };
Token Token::operator <(Token& tk){ 
    if((tk.tipo != tipo) && (tipo != ENTERO || tipo != FLOAT)) throw TypeError(tk.getNombreTipo(), this->getNombreTipo(), num_linea);    
    return tk; 
};
Token Token::operator +(Token& tk){ 
    if(tk.tipo == ENTERO && ENTERO == tipo) return Token(to_string(tk.parse<int>() + this->parse<int>()), num_linea);
...
Token Token::operator -(Token& tk){
    if(tk.tipo == ENTERO && tipo == ENTERO) return Token(to_string(tk.parse<int>() - this->parse<int>()), num_linea);
    else throw TypeError(tk.getNombreTipo(), this->getNombreTipo(), num_linea);
};
Token Token::operator *(Token& tk){ 
